...
public protocol PullToRefreshManagerDelegate: NSObjectProtocol {
    func pullToRefreshManagerDidStartLoading(_ controller: PullToRefreshManager,
...
open class PullToRefreshManager: NSObject {
    ...
    public init (
        ...
        stateConfig: StateConfiguration = StateConfiguration(
        thresholdInitiateLoading: 0, loaderFrame:
        CGRect(x: 0, y: -64, width: UIScreen.main.bounds.size.width, height: 64),
        thresholdStartLoading: -64)) {
        super.init()
        self.scrollView = scrollView
        self.delegate = delegate
        self.stateConfig = stateConfig
        self.scrollViewStateController = ScrollViewStateController(
