...
public protocol PaginationManagerDelegate: NSObjectProtocol {
    ...
    func paginationManagerDidStartLoading(_ controller: PaginationManager,
...
open class PaginationManager: NSObject {
    ...
    public init(scrollView: UIScrollView?,
                delegate: PaginationManagerDelegate?,
                stateConfig: StateConfiguration = StateConfiguration(thresholdInitiateLoading: 0,
                                                                     loaderFrame:
        CGRect(x: 0, y: 0, width: UIScreen.main.bounds.size.width, height: 64),
                                                                     thresholdStartLoading: 64)) {
        super.init()
        self.scrollView = scrollView
        self.delegate = delegate
        self.stateConfig = stateConfig
        self.scrollViewStateController = ScrollViewStateController(scrollView: scrollView,
                                                                   dataSource: self,
                                                                   delegate: self,
                                                                   showDefaultLoader: stateConfig.showDefaultLoader)
    }
    ...
    open func updateActivityIndicatorStyle(_ newStyle: UIActivityIndicatorView.Style) {
...
extension PaginationManager: ScrollViewStateControllerDataSource {
    ...
    public func stateControllerInsertLoaderInsets(_ startAnimation: Bool) -> UIEdgeInsets {
        ...
        newInset?.bottom += startAnimation ?
            self.stateConfig.loaderFrame.size.height : -self.stateConfig.loaderFrame.size.height
        return newInset!
    }
...
extension PaginationManager: ScrollViewStateControllerDelegate {
    public func stateControllerDidStartLoading(_ controller: ScrollViewStateController,
                                               onCompletion: @escaping CompletionHandler) {
